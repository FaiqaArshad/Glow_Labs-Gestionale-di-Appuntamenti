...
if (process.env.NODE_ENV === 'production') {
  // your production-only code here
  console.log("prod", process.env.PRODUCTION_CLIENT_URL)
}
else
console.log("dev")
...
console.log(refreshToken , ' ', refreshToken, "ggg")
  const generateDummyToken = (client) => {
    const token = jwt.sign(
      {
        id: client._id,
...
app.use(async (req, res, next) => {
  ...
  if (!accessToken && !refreshToken && !temporaryFacebookAccessToken) {
    ...
    try {
      ...
      console.log(process.env.PRODUCTION_CLIENT_ROOT, "process.env.PRODUCTION_CLIENT_ROOT")
